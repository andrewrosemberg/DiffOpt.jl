function moonlander_JMP(;target::Array{Float64}=[5.0, 5.0],nh::Int64=500, _I = 0.1)
    ## parameters
    if size(target) != (2,)
        error("The input matrix must be 2x1.")
    end
    m = 1.0
    g = 9.81
    D = 1.0
    ...
    @variable(model, Isp ∈ MOI.Parameter(_I))
    ...
    @expressions(model, begin
        ...
        ddtheta[k=0:nh], (1/Isp) * (D/2) * (F2[k] - F1[k])
    end)    
    @expressions(model, begin
        step,           tf / nh
    end)
